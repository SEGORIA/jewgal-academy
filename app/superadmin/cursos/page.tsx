  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <h1 style={{ fontFamily: "var(--serif)", fontWeight: 500, fontSize: 36, color: "#eef4f4", marginBottom: 6 }}>Programas</h1>
        <p style={{ color: "rgba(224,233,234,.4)", fontSize: 14 }}>Gestioná contenido, sesiones y alumnas de cada programa.</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: selected ? "300px 1fr" : "1fr", gap: 20, alignItems: "start" }}>
          ...
          {programs.map((p) => (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 600, color: "#eef4f4", flex: 1 }}>{p.name}</span>
                <button onClick={(e) => { e.stopPropagation(); togglePublish(p.slug) }}
                  style={{ background: "none", border: "none", cursor: "pointer", color: p.published ? "#6BBF8E" : "rgba(224,233,234,.25)", padding: 2 }}>
                  {p.published ? <Eye size={16} /> : <EyeOff size={16} />}
                </button>
              </div>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <span style={{ fontSize: 12, color: "rgba(224,233,234,.4)" }}>{p.isFree ? "Gratuito" : `$${p.price}`}</span>
                <span style={{ fontSize: 12, color: "rgba(224,233,234,.25)" }}>· {p.students} alumnas</span>
                <span style={{ marginLeft: "auto", fontSize: 11, color: p.published ? "#6BBF8E" : "rgba(224,233,234,.3)" }}>
        ...
        {selected && (
          <div style={{ ...card, padding: "28px 26px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22, paddingBottom: 20, borderBottom: "1px solid rgba(255,255,255,.06)" }}>
              <div>
                ...
                <h2 style={{ fontFamily: "var(--serif)", fontWeight: 500, fontSize: 22, color: "#eef4f4" }}>{selected.name}</h2>
              </div>
              <button onClick={() => setSelected(null)} style={{ background: "none", border: "none", color: "rgba(224,233,234,.3)", cursor: "pointer" }}>
                <X size={18} />
              </button>
            </div>
            ...
            <div style={{ display: "flex", gap: 4, marginBottom: 24, background: "rgba(255,255,255,.04)", borderRadius: 10, padding: 4 }}>
              {([
                ...
                <button key={key} onClick={() => setTab(key)} style={{
                  ...
                  color: tab === key ? selected.accent : "rgba(224,233,234,.4)",
                  transition: "all .18s",
                }}>
                  <Icon size={14} /> {label}
                </button>
            ...
            {tab === "materiales" && (
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <p style={{ fontSize: 13, color: "rgba(224,233,234,.4)" }}>0 materiales subidos</p>
                ...
                {showAddMat && (
                  <div style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(165,141,102,.18)", borderRadius: 12, padding: "20px 18px", marginBottom: 16 }}>
                    ...
                    <div style={{ border: "2px dashed rgba(165,141,102,.2)", borderRadius: 10, padding: "24px", textAlign: "center", cursor: "pointer", marginBottom: 12 }}>
                      ...
                      <p style={{ fontSize: 13, color: "rgba(224,233,234,.35)" }}>Arrastrá un archivo o hacé clic para seleccionar</p>
                      <p style={{ fontSize: 11, color: "rgba(224,233,234,.2)", marginTop: 4 }}>PDF, DOCX, MP4 · Máx. 100MB</p>
                    </div>
                    <div style={{ display: "flex", gap: 10 }}>
                      <button onClick={() => setShowAddMat(false)} style={{ flex: 1, background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 8, padding: "10px 0", fontSize: 13, color: "rgba(224,233,234,.5)", cursor: "pointer" }}>Cancelar</button>
                ...
                <div style={{ padding: "40px 0", textAlign: "center" }}>
                  ...
                  <p style={{ color: "rgba(224,233,234,.35)", fontSize: 14 }}>No hay materiales aún</p>
                </div>
              </div>
            )}
            ...
            {tab === "sesiones" && (
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <p style={{ fontSize: 13, color: "rgba(224,233,234,.4)" }}>0 sesiones programadas</p>
                ...
                {showAddSes && (
                  <div style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(165,141,102,.18)", borderRadius: 12, padding: "20px 18px", marginBottom: 16 }}>
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                      <button onClick={() => setShowAddSes(false)} style={{ flex: 1, background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 8, padding: "10px 0", fontSize: 13, color: "rgba(224,233,234,.5)", cursor: "pointer" }}>Cancelar</button>
                ...
                <div style={{ padding: "40px 0", textAlign: "center" }}>
                  ...
                  <p style={{ color: "rgba(224,233,234,.35)", fontSize: 14 }}>No hay sesiones programadas</p>
                </div>
              </div>
            )}
            ...
            {tab === "alumnos" && (
              <div style={{ padding: "40px 0", textAlign: "center" }}>
                ...
                <p style={{ color: "rgba(224,233,234,.35)", fontSize: 14 }}>No hay alumnas en este programa</p>
                <p style={{ color: "rgba(224,233,234,.2)", fontSize: 13, marginTop: 4 }}>Se inscribirán desde la página pública o desde el admin.</p>
              </div>
            )}
          </div>
        )}
