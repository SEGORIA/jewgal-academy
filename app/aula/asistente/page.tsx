...
import { MessageCircle, Send, Loader2, Plus, AlertCircle } from "lucide-react"
...
export default function AsistentePage() {
  ...
  async function sendMessage() {
    ...
    // Optimistic: agregar mensaje del usuario
    ...
    try {
      ...
      // Actualizar la conversación activa desde el header
      ...
      while (true) {
        ...
        setMessages((prev) =>
          prev.map((m) => (m.id === placeholderId ? { ...m, content: snap } : m))
        )
      }

      // Actualizar lista de conversaciones
      const convsRes = await fetch("/api/me/conversations")
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", gap: 24, height: "calc(100vh - 132px)" }}>
          ...
          {loadingConvs ? (
            ...
            conversations.map((c) => (
              <button
                ...
                onMouseEnter={(e) => { if (c.id !== activeId) (e.currentTarget as HTMLElement).style.background = "rgba(165,141,102,.07)" }}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", ...card, overflow: "hidden" }}>
        ...
        {/* Header del chat */}
        <div style={{
          padding: "16px 20px", borderBottom: "1px solid rgba(165,141,102,.1)",
          display: "flex", alignItems: "center", gap: 12, flexShrink: 0,
        }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: "linear-gradient(135deg,rgba(196,159,114,.25) 0%,rgba(167,109,97,.15) 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px" }}>
          {noEnrollment ? (
            <div style={{ padding: "40px 20px", textAlign: "center" }}>
              ...
              <p style={{ color: "var(--text-dim)", fontSize: 13, maxWidth: 320, margin: "0 auto" }}>
                Necesitás una inscripción activa en algún programa para usar el asistente.
              </p>
            </div>
          ) : messages.length === 0 ? (
            <div style={{ padding: "40px 20px", textAlign: "center" }}>
              <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(165,141,102,.1)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 18px" }}>
              ...
              <p style={{ color: "var(--text-dim)", fontSize: 13, maxWidth: 340, margin: "0 auto" }}>
        ...
        <div style={{
          padding: "12px 16px",
          borderTop: "1px solid rgba(165,141,102,.1)",
          ...
          <button
            ...
            style={{
              ...
              background: !input.trim() || streaming || noEnrollment
                ? "rgba(165,141,102,.1)"
                : "linear-gradient(135deg,#A76D61 0%,#C49F72 100%)",
              border: "none", cursor: !input.trim() || streaming || noEnrollment ? "not-allowed" : "pointer",
